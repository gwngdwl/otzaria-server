...
The DB directory is mounted **writable**: `seforim.db` ships in WAL journal mode, so SQLite creates
`seforim.db-shm` / `seforim.db-wal` next to it even though the server opens the DB content read-only.
